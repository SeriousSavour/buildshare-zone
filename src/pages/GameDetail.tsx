...
const GameDetail = () => {
  ...
  // Handle game_url: decide between raw HTML, direct URL, or proxied URL
  useEffect(() => {
    ...
    const isRawHtml =
      value.startsWith("<") || value.includes("<!DOCTYPE") || value.includes("<html") || value.includes("&lt;");

    const proxyUrl = import.meta.env.VITE_PROXY_WORKER_URL as string | undefined;

    if (isRawHtml) {
      // Raw HTML stored directly in game_url (from HTML file uploads)
      setHtmlContent(decodeHtmlEntities(value));
      setUseDirectUrl(false);
    } else {
      // It's some kind of URL
      const isSupabaseFile =
        value.includes(".supabase.co/storage/v1/object/public") || value.includes("/storage/v1/object/public");

      if (isSupabaseFile && proxyUrl) {
        // Use Cloudflare worker proxy to bypass iframe restrictions
        const proxied = `${proxyUrl}?url=${encodeURIComponent(value)}`;
        setHtmlContent(proxied);
        setUseDirectUrl(true);
      } else {
        // Plain external URL, loaded directly
  ...
  const handleLike = async () => {
    ...
    try {
      const { data: getUserData } = await supabase.rpc("get_user_by_session", {
        _session_token: sessionToken,
      });
                      ...
                      {isLoadingGame && (
                        <div className="absolute inset-0 z-30 bg-background/95 backdrop-blur-sm flex flex-col items-center justify-center gap-4 rounded-lg border-2 border-primary/50">
                          <div className="text-center space-y-4 max-w-md px-6">
                            ...
                            <p className="text-muted-foreground">Fetching game content through proxy server</p>
                          </div>
                        </div>
                      )}
                        ...
                        <div className="space-y-3 max-h-96 overflow-y-auto">
                          {comments.length === 0 ? (
                            ...
                            comments.map((comment) => (
                              <div
                                key={comment.id}
                                className="p-4 rounded-lg bg-card border border-border hover:border-primary/30 transition-all duration-200 shadow-sm"
                              >
                ...
                <Card className="h-full">
                  <CardContent className="pt-6 space-y-6">
                    ...
                    <div className="flex gap-2 pb-4 border-b border-border">
                      <Button variant={isLiked ? "default" : "outline"} className="flex-1" onClick={handleLike}>
