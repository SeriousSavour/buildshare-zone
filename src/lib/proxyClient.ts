...
const PROXY_ENTRY_URL = `${SUPABASE_URL}/functions/v1/proxy-entry`;
...
const proxyFetch: typeof fetch = async (input, init) => {
  ...
  if (url.includes('supabase.co') && !url.includes('/storage/') && !url.includes('/functions/v1/')) {
    ...
    const proxyUrl = `${PROXY_ENTRY_URL}?path=${encodeURIComponent(targetPath)}`;
    
    console.log(`[DOUBLE-PROXY] ${init?.method || 'GET'} ${targetPath}`);
    ...
    try {
      ...
      if (response.ok || response.status < 500) {
        console.log(`[DOUBLE-PROXY] ✓ ${response.status}`);
        return response;
      }
      
      // Only fall back on 500+ errors
      console.warn(`[DOUBLE-PROXY] ✗ ${response.status}, falling back`);
      return await fetch(input, init);
      
    } catch (error) {
      console.error(`[DOUBLE-PROXY] Failed:`, error);
